...
String where = " 1=1 ";   // 防止sql 搜索条件为： where and a=b 这样的错误
    where += " AND chuliren='"+request.getSession().getAttribute("username")+"' "; //  设置chuliren为当前登录用户
...
where+=" AND fenlei ='"+request.getParameter("fenlei")+"'"; } 
                 ...
                 if(request.getParameter("chuliren") != null && !"".equals( request.getParameter("chuliren") )){ where+=" AND chuliren LIKE '%"+request.getParameter("chuliren")+"%'"; } 
    List<HashMap> list = Query.make("shebeibaofei").where(where).order(orderby+" "+sort).page(15);
...
<div class="panel panel-default">
    ...
    分类
                            ...
                            <select class="form-control class_fenlei28" data-value="<%= request.getParameter("fenlei") !=null ? request.getParameter("fenlei") : "" %>" id="fenlei" name="fenlei">
...
<script>
$(".class_fenlei28").val($(".class_fenlei28").attr("data-value"))</script>
    ...
    处理人
                            ...
                            <input type="text" class="form-control" style="" name="chuliren" value="<%= request.getParameter("chuliren") !=null ? request.getParameter("chuliren") : "" %>"/>                        
</div>
...
<td> <% 
 HashMap mapshebeifenlei26 = new CommDAO().find("SELECT fenleimingcheng FROM shebeifenlei where id='"+map.get("fenlei")+"'"); 
 %><%= mapshebeifenlei26.get("fenleimingcheng") %> </td>
...
<td> <%= map.get("chuliren") %> </td>
                        ...
                        <td align="center">
                                                                                        ...
                                                                                        <a href="shebeibaofei_detail.jsp?id=<%= map.get("id") %>">
                                    详细
                                </a>
                                                        <a href="shebeibaofei_updt.jsp?id=<%= map.get("id") %>">修改</a>
                            <a href="shebeibaofei.jsp?a=delete&id=<%= map.get("id") %>" onclick="return confirm('真的要删除？')">删除</a>
                            <!--qiatnalijne-->
                        </td>
                    </tr>
                    <% } %>
