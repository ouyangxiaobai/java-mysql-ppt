...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"chuku",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update cailiaoxinxi set kucun =kucun-'"+request.getParameter("chukushuliang")+"' where id ='"+request.getParameter("cailiaoxinxiid")+"'");
    ...
    String charuid = new CommDAO().update(request,"chuku",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("chuku").find(Request.get("id"));
    
    new CommDAO().delete(request,"chuku");  // 执行删除命令
