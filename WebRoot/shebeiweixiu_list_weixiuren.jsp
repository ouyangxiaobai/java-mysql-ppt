...
String where = " 1=1 ";   // 防止sql 搜索条件为： where and a=b 这样的错误
    where += " AND weixiuren='"+request.getSession().getAttribute("username")+"' "; //  设置weixiuren为当前登录用户
...
where+=" AND fenlei ='"+request.getParameter("fenlei")+"'"; } 
                         if(request.getParameter("weixiuren") != null && !"".equals( request.getParameter("weixiuren") )){ where+=" AND weixiuren LIKE '%"+request.getParameter("weixiuren")+"%'"; } 
...
$(".class_fenlei25").val($(".class_fenlei25").attr("data-value"))</script>
    ...
    维修人
                            ...
                            <input type="text" class="form-control" style="" name="weixiuren" value="<%= request.getParameter("weixiuren") !=null ? request.getParameter("weixiuren") : "" %>"/>                        
</div>
...
<td> <% 
 HashMap mapshebeifenlei19 = new CommDAO().find("SELECT fenleimingcheng FROM shebeifenlei where id='"+map.get("fenlei")+"'"); 
 %><%= mapshebeifenlei19.get("fenleimingcheng") %> </td>
...
<td> <%= map.get("weixiuren") %> </td>
                        ...
                        <td align="center">
                            ...
                            <a href="shebeiweixiu.jsp?a=delete&id=<%= map.get("id") %>" onclick="return confirm('真的要删除？')">删除</a>
                            <!--qiatnalijne-->
                        </td>
                    </tr>
                    <% } %>
