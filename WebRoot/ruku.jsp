...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"ruku",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update cailiaoxinxi set kucun =kucun+'"+request.getParameter("rukushuliang")+"' where id ='"+request.getParameter("cailiaoxinxiid")+"'");
    ...
    String charuid = new CommDAO().update(request,"ruku",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("ruku").find(Request.get("id"));
    
    new CommDAO().delete(request,"ruku");  // 执行删除命令
