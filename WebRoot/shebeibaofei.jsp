...
if(a.equals("insert")){
    // 执行插入设备报废模块,表名：shebeibaofei
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                            
    String charuid = new CommDAO().insert(request,"shebeibaofei",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update shebeixinxi set zhuangtai ='已报废' where id ='"+request.getParameter("shebeixinxiid")+"'");
    ...
    // 执行更新设备报废模块,表名：shebeibaofei
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                                                

    String charuid = new CommDAO().update(request,"shebeibaofei",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("shebeibaofei").find(Request.get("id"));
    
    new CommDAO().delete(request,"shebeibaofei");  // 执行删除命令
