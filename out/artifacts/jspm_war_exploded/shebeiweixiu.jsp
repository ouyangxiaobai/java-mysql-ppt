...
if(a.equals("insert")){
    // 执行插入设备维修模块,表名：shebeiweixiu
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                        ext.put("weixiuneirong" , util.DownloadRemoteImage.run(Request.get("weixiuneirong")));// 将前台编辑器内容的图片远程下载下来并保存在本地中    
    String charuid = new CommDAO().insert(request,"shebeiweixiu",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update shebeixinxi set zhuangtai ='已维修' where id ='"+request.getParameter("shebeixinxiid")+"'");
    ...
    // 执行更新设备维修模块,表名：shebeiweixiu
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                                                                ext.put("weixiuneirong" , util.DownloadRemoteImage.run(Request.get("weixiuneirong")));// 将前台编辑器内容的图片远程下载下来并保存在本地中                

    String charuid = new CommDAO().update(request,"shebeiweixiu",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("shebeiweixiu").find(Request.get("id"));
    
    new CommDAO().delete(request,"shebeiweixiu");  // 执行删除命令
