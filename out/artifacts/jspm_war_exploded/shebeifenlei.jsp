...
if(a.equals("insert")){
    // 执行插入设备分类模块,表名：shebeifenlei
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
        
    String charuid = new CommDAO().insert(request,"shebeifenlei",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    // 执行更新设备分类模块,表名：shebeifenlei
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
                    

    String charuid = new CommDAO().update(request,"shebeifenlei",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("shebeifenlei").find(Request.get("id"));
    
    new CommDAO().delete(request,"shebeifenlei");  // 执行删除命令
